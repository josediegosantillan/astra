...
function initModals() {
    ...
    document.querySelectorAll('[data-modal-open]').forEach(btn => {
        btn.addEventListener('click', (e) => {
            ...
            if (modal) {
                ...
                if (document.querySelectorAll('.site-modal:not(.hidden)').length === 0) {
                    document.body.style.overflow = 'hidden';
                }

                // Manage focus: focus first autofocus element, or modal container
                const auto = modal.querySelector('[data-autofocus]') || modal.querySelector('button, a, [tabindex], input, textarea');
                if (auto) auto.focus();
                else modal.focus();
                // Setup basic focus trap by remembering first and last tabbables
                const tabbables = Array.from(modal.querySelectorAll('a[href], button:not([disabled]), input:not([disabled]), textarea:not([disabled]), select:not([disabled]), [tabindex]:not([tabindex="-1"])'));
                if (tabbables.length) {
                    modal._firstTabbable = tabbables[0];
                    modal._lastTabbable = tabbables[tabbables.length - 1];
                }
            }
        });
    });
    ...
    document.querySelectorAll('[data-modal-close]').forEach(btn => {
        btn.addEventListener('click', (e) => {
            ...
            if (modal) {
                const dialog = modal.querySelector('.modal-dialog');
                // remove overlay after dialog hide completes
                const finishHide = () => {
                ...
                // restore body scroll only if no other visible modals
                if (document.querySelectorAll('.site-modal:not(.hidden)').length === 0) {
                    document.body.style.overflow = 'auto';
                }
                // restore focus to the element that opened the modal if available
                const openerId = modal.getAttribute('data-opened-by');
                if (openerId) {
                    const opener = document.getElementById(openerId) || document.querySelector('[data-modal-open="' + openerId + '"]');
                    if (opener) opener.focus();
                }
                // cleanup stored tabbables
                delete modal._firstTabbable;
                delete modal._lastTabbable;
            }
        });
    });
    ...
    // Basic focus trap: keep tabbing inside open modal
    document.addEventListener('keydown', function(e) {
        if (e.key === 'Tab') {
            const openModal = document.querySelector('.site-modal:not(.hidden)');
            if (!openModal) return;
            const first = openModal._firstTabbable;
            const last = openModal._lastTabbable;
